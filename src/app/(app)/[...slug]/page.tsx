...
function MobileCard({ children }: { children: ReactNode }) {
  return <div className="space-y-2 p-4 text-sm">{children}</div>;
}
...
function ErrorText({ error }: { error: unknown }) {
  const value = normalizeError(error as Parameters<typeof normalizeError>[0]);
  return value ? <p className="text-loss text-sm">{value.message}</p> : null;
}
...
const depositColumns: DataTableColumn<Deposit>[] = [
  { id: "date", header: "Date", cell: (r) => date(r.createdAt), nowrap: true },
  {
    id: "amount",
    header: "Amount",
    cell: (r) => <Money value={r.amount} showCurrency />,
    nowrap: true,
  },
  {
    id: "bonus",
    header: "Bonus",
    cell: (r) => (
      <Money value={r.bonusAmount} showCurrency variant="positive" />
    ),
    nowrap: true,
  },
...
function DepositMobile({ row }: { row: Deposit }) {
  return (
    <MobileCard>
      <Row label="Amount">
        <Money value={row.amount} showCurrency />
      </Row>
      <Row label="Bonus">
        <Money value={row.bonusAmount} showCurrency />
      </Row>
      <Row label="Status">
...
const withdrawalColumns: DataTableColumn<Withdrawal>[] = [
  { id: "date", header: "Date", cell: (r) => date(r.createdAt), nowrap: true },
  { id: "wallet", header: "Wallet", cell: (r) => title(r.walletType) },
  {
    id: "amount",
    header: "Amount",
    cell: (r) => <Money value={r.amount} showCurrency />,
  ...
  },
  {
    id: "net",
    header: "Net",
    cell: (r) => <Money value={r.netAmount} showCurrency />,
    nowrap: true,
  },
  {
    id: "status",
...
function WithdrawalMobile({ row }: { row: Withdrawal }) {
  return (
    <MobileCard>
      <Row label="Wallet">{title(row.walletType)}</Row>
      <Row label="Amount">
        <Money value={row.amount} showCurrency />
      </Row>
      <Row label="Net">
        <Money value={row.netAmount} showCurrency />
      </Row>
      <Row label="Status">
        <StatusBadge status={row.status} />
      </Row>
...
function WithdrawPage() {
  const [page, setPage] = useState(1);
  const query = useWithdrawalsQuery(pageArgs(page));
  const [create, mutation] = useCreateWithdrawalMutation();
  ...
  return (
    <>
      <PageHeader
        title="Withdraw"
        description="Request a withdrawal from an eligible wallet."
      />
      <Card className="mb-6">
        <CardHeader>
          <CardTitle>New withdrawal</CardTitle>
          <CardDescription>
            The backend calculates fees, penalties, and the final net amount.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={submit} className="grid gap-4 md:grid-cols-3">
            <div className="space-y-2">
              ...
              <select
                ...
                onChange={(e) =>
                  setWalletType(
                    e.target.value as keyof typeof WithdrawalWalletType,
                  )
                }
              >
                {Object.keys(WithdrawalWalletType).map((v) => (
      ...
      <DataTable
        ...
        onRetry={query.refetch}
        getRowId={(r) => r.id}
        renderMobileCard={(r) => <WithdrawalMobile row={r} />}
        onPageChange={setPage}
      />
    </>
  );
...
function TransactionsPage() {
  const [page, setPage] = useState(1);
  const q = useLedgersQuery(pageArgs(page));
  const columns: DataTableColumn<Ledger>[] = [
    { id: "date", header: "Date", cell: (r) => date(r.createdAt) },
    { id: "type", header: "Type", cell: (r) => title(r.type) },
    {
      id: "description",
      header: "Description",
    ...
    },
    {
      id: "credit",
      header: "Credit",
      cell: (r) => <Money value={r.credit} variant="positive" />,
    },
    {
      id: "debit",
      header: "Debit",
      cell: (r) => <Money value={r.debit} variant="negative" />,
    },
    {
      id: "balance",
      header: "Balance",
      cell: (r) => <Money value={r.afterBalance} />,
    },
  ];
  return (
    <>
      <PageHeader
        title="Transactions"
        description="Your append-only wallet ledger."
      />
      <DataTable
        columns={columns}
        page={q.data}
        ...
        renderMobileCard={(r) => (
          <MobileCard>
            ...
            <Row label="Balance">
              <Money value={r.afterBalance} />
            </Row>
            <Row label="Date">{date(r.createdAt)}</Row>
          </MobileCard>
        )}
        onPageChange={setPage}
      />
...
function ProfitFlow() {
  const steps = [
    {
      label: "1% of your Principal",
      value: "USDT",
      hint: "Daily auto-trade amount",
    },
    { label: "Deployed to market", value: "185%", hint: "Gross return" },
    {
      label: "Profit split",
      value: "60% You / 40% Admin",
      hint: "Net distribution",
    },
  ];
  return (
    <Card className="mb-6">
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          <TrendingUp className="size-5" /> How auto trading works
        </CardTitle>
        <CardDescription>
          One daily session uses 1% of your Principal to generate profit and
          shares it 60/40. Your Principal doubles in ~90 days.
        </CardDescription>
      </CardHeader>
      <CardContent>
    ...
    <div className="flex items-center justify-between rounded-lg border p-3">
      ...
      <div className="text-right">
        <p className="text-sm font-medium">
          <Money value={trade.tradeAmount} />
        </p>
        <p className="text-xs">
          <Money value={trade.profit} variant="positive" />
        </p>
      </div>
    </div>
  );
...
const recentTradeColumns: DataTableColumn<Trade>[] = [
  ...
  { id: "type", header: "Session", cell: (r) => title(r.tradeType) },
  {
    id: "amount",
    header: "Amount",
    cell: (r) => <Money value={r.tradeAmount} />,
    nowrap: true,
  },
  {
    id: "profit",
    header: "Profit",
    cell: (r) => <Money value={r.profit} variant="positive" />,
    nowrap: true,
  },
  {
    id: "status",
...
function RecentTradesList() {
  ...
  return (
    <Card className="h-full">
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          <Clock className="size-5" /> Your recent trades
        </CardTitle>
      ...
      <CardContent>
        <DataTable
          ...
          renderMobileCard={(r) => (
            <MobileCard>
              <Row label="Session">{title(r.tradeType)}</Row>
              <Row label="Amount">
                <Money value={r.tradeAmount} />
              </Row>
              <Row label="Profit">
                <Money value={r.profit} variant="positive" />
              </Row>
              <Row label="Status">
                <StatusBadge status={r.status} />
              </Row>
              <Row label="Entry">{date(r.entryTime)}</Row>
            </MobileCard>
          )}
        />
      </CardContent>
...
function TradingPage() {
  ...
  const [toggle, mutation] = useToggleAutoTradeMutation();
  const enabled = dashboard.data?.profile.autoTradeStatus;
  async function change() {
    try {
      const result = await toggle().unwrap();
      toast.success(
        `Auto trading ${result.autoTradeStatus ? "enabled" : "disabled"}.`,
      );
    } catch (e) {
      toast.error(
        normalizeError(e as Parameters<typeof normalizeError>[0])?.message,
      );
    }
  }
  return (
    <>
      <PageHeader
        title="Auto Trading"
        description="Control eligibility for future automated trade sessions."
      />

      <Card className="mb-6">
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            <ChartCandlestick className="size-5" /> Live Trading Engine
          </CardTitle>
          <CardDescription>
            Real-time AI market execution and trading signals.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <LiveTradingChart showHeading={false} />
        </CardContent>
      </Card>
      <Card className="mb-6">
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            <ChartCandlestick className="size-5" /> Auto-trade status
          </CardTitle>
          <CardDescription>
            Toggling does not create a trade; it changes eligibility for the
            next session.
          </CardDescription>
        </CardHeader>
        <CardContent className="flex items-center justify-between gap-4">
          <StatusBadge status={enabled ? "ACTIVE" : "INACTIVE"} />
          <Button onClick={change} disabled={mutation.isLoading}>
            {enabled ? "Disable auto trading" : "Enable auto trading"}
          </Button>
        </CardContent>
      </Card>
      <ProfitFlow />
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
        <StatCard
          label="Total trades"
          value={stats.data?.totalTrades ?? 0}
        ...
        <StatCard
          label="Trade volume"
          value={
            <Money value={String(stats.data?.totalVolume ?? 0)} showCurrency />
          }
          loading={stats.isLoading}
        />
        <StatCard
          ...
          value={
            <Money
              value={String(stats.data?.totalUserProfit ?? 0)}
              showCurrency
              variant="positive"
            />
          }
...
const tradeColumns: DataTableColumn<Trade>[] = [
  { id: "date", header: "Entry", cell: (r) => date(r.entryTime) },
  { id: "type", header: "Session", cell: (r) => title(r.tradeType) },
  {
    id: "amount",
    header: "Amount",
  ...
  },
  {
    id: "profit",
    header: "Profit",
    cell: (r) => <Money value={r.profit} variant="signed" />,
  },
  {
    id: "status",
    header: "Status",
...
function TradeHistoryPage() {
  const [page, setPage] = useState(1);
  const q = useTradesQuery(pageArgs(page));
  return (
    <>
      <PageHeader
        title="Trade History"
        description="Executed and pending automated trades."
      />
      <DataTable
        ...
        renderMobileCard={(r) => (
          <MobileCard>
            ...
            <Row label="Profit">
              <Money value={r.profit} variant="signed" />
            </Row>
            <Row label="Status">
              <StatusBadge status={r.status} />
            </Row>
...
function ReferralPage() {
  ...
  const columns: DataTableColumn<Referral>[] = [
    ...
    { id: "team", header: "Team size", cell: (r) => r.teamSize },
    {
      ...
      cell: (r) => <Money value={r.directDepositAmount} />,
    },
  ];
  async function copy() {
  ...
  return (
    <>
      <PageHeader
        title="Referrals"
        description="Share your link and track your direct network."
      />
      <Card className="mb-6">
        <CardHeader>
      ...
      <div className="mb-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <StatCard
          ...
          value={stats.data?.totalReferrals ?? 0}
          icon={Users}
          loading={stats.isLoading}
        />
        <StatCard
          label="Direct referrals"
          value={stats.data?.directReferrals ?? 0}
          loading={stats.isLoading}
        />
        <StatCard
          label="Bonuses"
          value={stats.data?.totalBonuses ?? 0}
          loading={stats.isLoading}
        />
        <StatCard
          label="Bonus earned"
          value={<Money value={String(stats.data?.totalBonusAmount ?? 0)} />}
          accent="profit"
          loading={stats.isLoading}
        />
      </div>
...
function ReferralTreePage() {
  ...
  return (
    <>
      <PageHeader
        title="Referral Tree"
        description="Your network structure through five levels."
      />
      {q.isLoading ? (
        <Card>
...
const bonusColumns: DataTableColumn<ReferralBonus>[] = [
  { id: "date", header: "Date", cell: (r) => date(r.createdAt) },
  { id: "level", header: "Level", cell: (r) => r.level },
  {
    id: "deposit",
    header: "Deposit",
    cell: (r) => <Money value={r.depositAmount} />,
  },
  { id: "rate", header: "Rate", cell: (r) => `${r.bonusPercentage}%` },
  {
    id: "bonus",
    header: "Bonus",
    cell: (r) => <Money value={r.bonusAmount} variant="positive" />,
  },
  {
    id: "status",
    header: "Status",
...
function ReferralEarningsPage() {
  const [page, setPage] = useState(1);
  const q = useReferralBonusesQuery(pageArgs(page));
  return (
    <>
      <PageHeader
        title="Referral Earnings"
        description="Bonuses generated by qualifying network deposits."
      />
      <DataTable
        ...
        renderMobileCard={(r) => (
          <MobileCard>
            <Row label="Level">{r.level}</Row>
            <Row label="Deposit">
              <Money value={r.depositAmount} />
            </Row>
            <Row label="Bonus">
              <Money value={r.bonusAmount} />
            </Row>
            <Row label="Status">
              <StatusBadge status={r.status} />
            </Row>
...
function RanksPage() {
  ...
  const columns: DataTableColumn<RankHistoryEntry>[] = [
    { id: "date", header: "Changed", cell: (r) => date(r.changedAt) },
    {
      id: "from",
      header: "Previous",
      cell: (r) => <RankBadge rank={r.previousLevel} />,
    },
    {
      id: "to",
      header: "New rank",
      cell: (r) => <RankBadge rank={r.newLevel} />,
    },
    { id: "reason", header: "Reason", cell: (r) => r.changeReason ?? "—" },
  ];
  return (
    <>
      <PageHeader
        title="Ranks"
        description="Current achievement and promotion history."
      />
      <div className="mb-6 grid gap-4 sm:grid-cols-3">
        <StatCard
          label="Current rank"
          value={<RankBadge rank={q.data?.currentRank} size="md" />}
          icon={Trophy}
          loading={q.isLoading}
        />
        <StatCard
          label="Direct referrals"
          value={q.data?.rankDetails?.directReferrals ?? 0}
          loading={q.isLoading}
        />
        <StatCard
          label="Team size"
      ...
      <DataTable
        ...
        renderMobileCard={(r) => (
          <MobileCard>
            <Row label="Previous">
              <RankBadge rank={r.previousLevel} />
            </Row>
            <Row label="New">
              <RankBadge rank={r.newLevel} />
            </Row>
            <Row label="Date">{date(r.changedAt)}</Row>
          </MobileCard>
        )}
        hidePagination
      />
...
function CycleBonusPage() {
  const [page, setPage] = useState(1);
  const q = useCycleBonusesQuery(pageArgs(page));
  const columns: DataTableColumn<CycleBonus>[] = [
    { id: "cycle", header: "Cycle", cell: (r) => `#${r.cycleNumber}` },
    {
      id: "rank",
      header: "Rank",
      cell: (r) => <RankBadge rank={r.rankLevel} />,
    },
    {
      id: "period",
      header: "Period",
      cell: (r) => `${date(r.cycleStartDate)} – ${date(r.cycleEndDate)}`,
    },
    {
      id: "amount",
      header: "Total",
      cell: (r) => <Money value={r.totalAmount} variant="positive" />,
    },
    {
      id: "status",
      header: "Status",
  ...
  return (
    <>
      <PageHeader
        title="Cycle Bonus"
        description="Rank and cycle rewards from completed eligibility periods."
      />
      <DataTable
        columns={columns}
        page={q.data}
        ...
        renderMobileCard={(r) => (
          <MobileCard>
            <Row label="Cycle">#{r.cycleNumber}</Row>
            <Row label="Rank">
              <RankBadge rank={r.rankLevel} />
            </Row>
            <Row label="Total">
...
function PoolBonusPage() {
  ...
  const columns: DataTableColumn<PoolBonusRequest>[] = [
    ...
    { id: "type", header: "Type", cell: (r) => title(r.requestType) },
    {
      id: "requested",
      header: "Requested",
      cell: (r) => <Money value={r.requestedAmount} showCurrency />,
      nowrap: true,
    },
    {
      id: "approved",
      ...
      cell: (r) =>
        r.approvedAmount ? (
          <Money value={r.approvedAmount} showCurrency />
        ) : (
          "—"
        ),
      nowrap: true,
  ...
  return (
    <>
      ...
      <DataTable
        ...
        renderMobileCard={(r) => (
          <MobileCard>
            <Row label="Type">{title(r.requestType)}</Row>
            <Row label="Requested">
              <Money value={r.requestedAmount} showCurrency />
            </Row>
            <Row label="Approved">
              {r.approvedAmount ? (
                <Money value={r.approvedAmount} showCurrency />
...
function DepositBonusPage() {
  const [page, setPage] = useState(1);
  const q = useDepositsQuery(pageArgs(page));
  const bonuses = q.data
    ? {
        ...q.data,
        items: q.data.items.filter((r) => Number(r.bonusAmount) > 0),
      }
    : undefined;
  const shown =
    bonuses?.items.reduce((sum, r) => sum + Number(r.bonusAmount), 0) ?? 0;
  return (
    <>
      <PageHeader
        title="Deposit Bonus"
        description="Actual bonuses recorded on your deposit history."
      />
      <div className="mb-6">
        <StatCard
          label="Bonuses on this page"
          value={<Money value={String(shown)} showCurrency />}
          hint="Derived from the loaded deposit records"
          icon={Gift}
          loading={q.isLoading}
        />
      </div>
      <DataTable
        ...
        renderMobileCard={(r) => <DepositMobile row={r} />}
        onPageChange={setPage}
        emptyState={{ title: "No deposit bonuses on this page" }}
      />
    </>
  );
}
...
function NotificationsPage() {
  const [page, setPage] = useState(1);
  const q = useNotificationsQuery(pageArgs(page));
  const [mark, marking] = useMarkNotificationReadMutation();
  const [markAll, markingAll] = useMarkAllNotificationsReadMutation();
  const columns: DataTableColumn<AppNotification>[] = [
    {
      id: "status",
      header: "Status",
      cell: (r) => <StatusBadge status={r.read ? "READ" : "UNREAD"} />,
    },
    {
      id: "notification",
      header: "Notification",
      cell: (r) => (
        <div>
          ...
          <p className="text-muted-foreground max-w-xl text-sm">{r.message}</p>
        </div>
      ),
    },
    { id: "date", header: "Received", cell: (r) => date(r.createdAt) },
    {
      id: "action",
      header: "",
      cell: (r) =>
  ...
  return (
    <>
      <PageHeader
        title="Notifications"
        description="Account, transaction, trading, and reward alerts."
        actions={
          <Button
            variant="outline"
      ...
      <DataTable
        ...
        loading={q.isLoading}
        fetching={q.isFetching}
        error={normalizeError(q.error ?? marking.error ?? markingAll.error)}
        onRetry={q.refetch}
        getRowId={(r) => r.id}
        renderMobileCard={(r) => (
          <MobileCard>
            <Row label="Status">
              <StatusBadge status={r.read ? "READ" : "UNREAD"} />
            </Row>
            <p className="font-medium">{r.title}</p>
            <p className="text-muted-foreground">{r.message}</p>
            <Row label="Received">{date(r.createdAt)}</Row>
            {!r.read && (
              <Button size="sm" variant="outline" onClick={() => mark(r.id)}>
                Mark read
              </Button>
            )}
          </MobileCard>
...
function SettingsPage() {
  const [change, mutation] = useChangePasswordMutation();
  ...
  async function submit(e: FormEvent) {
    ...
    try {
      await change({ oldPassword, newPassword }).unwrap();
      toast.success("Password changed. Sign in again on other sessions.");
      setOld("");
      setNew("");
      setConfirm("");
    } catch (error) {
  ...
  return (
    <>
      <PageHeader
        title="Settings"
        description="Security controls supported by your account."
      />
      <Card className="max-w-2xl">
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            <ShieldCheck className="size-5" /> Change password
          </CardTitle>
          <CardDescription>
            Changing your password revokes all existing refresh tokens.
          </CardDescription>
        </CardHeader>
        <CardContent>
...
function SupportTicketsPage() {
  ...
  const [helpType, setHelpType] = useState<
    "GENERAL_INQUIRY" | "TECHNICAL_SUPPORT" | "ACCOUNT_HELP" | "FEEDBACK"
  >("GENERAL_INQUIRY");
  ...
  async function handleCreate(e: FormEvent) {
    ...
    try {
      const ticket = await create({
        subject,
        message,
        priority: helpTypeToPriority[helpType],
        attachments,
      }).unwrap();
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-muted-foreground text-xs">
              {priorityToHelpType[r.priority] || r.priority}
            </p>
          </div>
        </div>
      ),
          ...
          <CardContent className="pt-6">
            <form onSubmit={handleCreate} className="space-y-6">
              ...
              <div className="space-y-2">
                ...
                />
                <p className="text-muted-foreground text-xs">
                  JPEG, PNG, GIF or WebP. Max 5 images, 5MB each.
                </p>
...
function TicketDetailPage({
  ...
  async function handleReply(e: FormEvent) {
    ...
    try {
      await sendReply({
        id: ticketId as UUID,
        body: { message: reply, attachments: replyAttachments },
      }).unwrap();
      setReply("");
      setReplyAttachments([]);
          ...
          <CardContent className="pt-6">
            <form onSubmit={handleReply} className="space-y-4">
              ...
              <div className="flex items-center justify-between">
                ...
                <Button
                  type="submit"
                  disabled={replyMut.isLoading || !reply.trim()}
...
function MyPostsPage() {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    if (!editId && !imageFile)
      return toast.error("Image is required for new posts.");
    try {
