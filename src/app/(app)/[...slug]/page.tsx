...
function WithdrawPage() {
  ...
  async function submit(e: FormEvent) { e.preventDefault(); if (!(Number(amount) > 0) || !walletAddress.trim()) return toast.error("Enter a positive amount and destination address."); try { await create({ amount, walletAddress, walletType: WithdrawalWalletType[walletType] }).unwrap(); toast.success("Withdrawal request submitted."); setAmount(""); } catch (error) { toast.error(normalizeError(error as Parameters<typeof normalizeError>[0])?.message); } }
